...
#include <algorithm>
#include <string> /* Kostas */
#include <limits.h> /* Kostas */
#include <sstream> /* Kostas */
#include <set> /* Kostas */
#include <stdlib.h> /* Kostas */
#include <stdio.h> /* Kostas */
#include <algorithm> /* Kostas */
#include <list> /* Kostas */
#include <vector> /* Kostas */
//#include <wnb/core/wordnet.hh> /* Kostas */
//#include <wnb/nltk_similarity.hh> /* Kostas */
using namespace std;
//using namespace wnb;
using std::max;
...
/**
* Get all the english characters in a set
*/
set<string> getAlphabet(){/* Kostas */
	set<string> alphabet;
	alphabet.insert("a"); alphabet.insert("b"); alphabet.insert("c"); alphabet.insert("d"); alphabet.insert("e");
	alphabet.insert("f"); alphabet.insert("g"); alphabet.insert("h"); alphabet.insert("i"); alphabet.insert("j");
	alphabet.insert("k"); alphabet.insert("l"); alphabet.insert("m"); alphabet.insert("n"); alphabet.insert("o");
	alphabet.insert("p"); alphabet.insert("q"); alphabet.insert("r"); alphabet.insert("s"); alphabet.insert("t");
	alphabet.insert("u"); alphabet.insert("v"); alphabet.insert("w"); alphabet.insert("x"); alphabet.insert("y");
	alphabet.insert("z"); alphabet.insert("A"); alphabet.insert("B"); alphabet.insert("C"); alphabet.insert("D");
	alphabet.insert("E"); alphabet.insert("F"); alphabet.insert("G"); alphabet.insert("H"); alphabet.insert("I");
	alphabet.insert("J"); alphabet.insert("K"); alphabet.insert("L"); alphabet.insert("M"); alphabet.insert("N");
	alphabet.insert("O"); alphabet.insert("P"); alphabet.insert("Q"); alphabet.insert("R"); alphabet.insert("S");
	alphabet.insert("T"); alphabet.insert("U"); alphabet.insert("V"); alphabet.insert("W"); alphabet.insert("X");
	alphabet.insert("Y"); alphabet.insert("Z");
	return alphabet;
}

/**
* Produce all words with the edit distance provided
*/
void produceSimilarityWords(set<string> &wordSet, int similarity){/* Kostas */

	//if similarity is zero then we do not need to calculate any word
	if(similarity == 0){
		return;
	}

	//define the three sets that will hold the derived words
	set<string> deleteSet;
	set<string> addSet;
	set<string> substituteSet;
	set<string> alphabet = getAlphabet();

	//for each word in the wordSet
	set<string>::iterator typeOneIt;
	for(typeOneIt=wordSet.begin(); typeOneIt!=wordSet.end(); ++typeOneIt){
		string word = *typeOneIt;
		int wordLength = strlen((word).c_str());
		//for each position in the word
		for(int pos = 0; pos < wordLength; ++pos){
			//delete the character in that position
			string newWord = word.substr(0, pos) + word.substr(pos+1, wordLength);
			//and hold that word
			deleteSet.insert(newWord);
		}
	}

	//for each word in the wordSet
	set<string>::iterator typeTwoIt;
	for (typeTwoIt=wordSet.begin(); typeTwoIt!=wordSet.end(); ++typeTwoIt){
		string word = *typeTwoIt;
		int wordLength = strlen((word).c_str());
		//for each character in the English language
		set<string>::iterator alphabetIt;
		for(alphabetIt=alphabet.begin(); alphabetIt!=alphabet.end(); ++alphabetIt){
			string character = *alphabetIt; 
			//for each position in the word
			for(int pos = 0; pos < wordLength; ++pos){
				//add that character in that position
				string newWord = word.substr(0, pos) + character + word.substr(pos, wordLength);
				//and hold that word
				addSet.insert(newWord);
			}
			//add also at the beginning and at the end of the word
			addSet.insert(character+word);
			addSet.insert(word+character);
		}
	}

	//for each word in the wordSet
	set<string>::iterator typeThreeIt;
	for (typeThreeIt=wordSet.begin(); typeThreeIt!=wordSet.end(); ++typeThreeIt){
		string word = *typeThreeIt;
		int wordLength = strlen((word).c_str());
		//for each character in the English language
		set<string>::iterator alphabetIt;
		for(alphabetIt=alphabet.begin(); alphabetIt!=alphabet.end(); ++alphabetIt){
			...
			//for each position in the word
			for(int pos = 0; pos < wordLength; ++pos){
				//substitute that character in that position
				string newWord = word.substr(0, pos) + character + word.substr(pos+1, wordLength);
				//and hold that word
				substituteSet.insert(newWord);
			}
		}
	}

	//add words in deleteSet and addSet in the wordSet set
	wordSet.insert(deleteSet.begin(), deleteSet.end());
	wordSet.insert(addSet.begin(), addSet.end());
	wordSet.insert(substituteSet.begin(), substituteSet.end());

	//call the same function with the new set of words and one reduced similarity
	produceSimilarityWords(wordSet, similarity-1);
...
*/
void produceWordNetWords(set<string> &wordSet){/* Kostas */
	...
	wordSet.insert(synWords.begin(), synWords.end());TODO*/
}

/**
* Given a set of words and the transofration needed return a string with the produced words
*/
string getExpandedWords(set<string> words, string modifier, int number){/* Kostas */
	if(modifier.find("with edit distance") != std::string::npos){
		produceSimilarityWords(words, number);
	} else if(modifier.find("with similarity") != std::string::npos){
...
/**
* Given an SQL query return a query that has flattened occurences of 'with edit distance' and 'with similarity' modifiers
*/
string expandQuery(char* packet){/* Kostas */
	char* arrayQ = new char[strlen(packet)];
	strcpy(arrayQ, packet);
	set<string> words;
	string number, modifier, newQuery;
	unsigned int j=0;
	for(unsigned int i=0; i<strlen(arrayQ); ++i){
		if(i+9<strlen(arrayQ) && (arrayQ[i] == 'a' || arrayQ[i] == 'A') && (arrayQ[i+1] == 'g' || arrayQ[i+1] == 'G') && (arrayQ[i+2] == 'a' || arrayQ[i+2] == 'A')
		 && (arrayQ[i+3] == 'i' || arrayQ[i+3] == 'I') && (arrayQ[i+4] == 'n' || arrayQ[i+4] == 'N') && (arrayQ[i+5] == 's' || arrayQ[i+5] == 'S')
		 && (arrayQ[i+6] == 't' || arrayQ[i+5] == 'T') && (arrayQ[i+7] == '(') && (arrayQ[i+8] == '\'') ){
			j = i+9;
			//proceed until the end of the against argument
			string word;
			while(j<strlen(arrayQ) && arrayQ[j] != '\''){
				//keep the distinct words
				if(arrayQ[j] != ' '){
					word += arrayQ[j];
				} else {
					string temp = word;
			...
			string temp = word;//keep the last word also
			words.insert(temp);
			if(j<strlen(arrayQ)){
				j++;//get pass the '
			} else {
			...
			//proceed until the ) is found -- keep number and modifier
			while(j<strlen(arrayQ) && arrayQ[j] != ')'){
				if(arrayQ[j] == '0' || arrayQ[j] == '1' || arrayQ[j] == '2' || arrayQ[j] == '3' || arrayQ[j] == '4'
				       || arrayQ[j] == '5' || arrayQ[j] == '6' || arrayQ[j] == '7' || arrayQ[j] == '8' || arrayQ[j] == '9'){
					number += arrayQ[j];
				} else {
					modifier += arrayQ[j];
				}
			...
			stringstream(number) >> distance;
			//populate the new part
			newQuery += "against('";
			//keep copy of the modifier
			string modifierCopy = modifier;
			std::transform(modifier.begin(), modifier.end(), modifier.begin(), ::tolower);
			//if modifier equals with edit distance or with similarity expand
			if(modifier.find("with edit distance") != std::string::npos || modifier.find("with similarity") != std::string::npos){
				newQuery += getExpandedWords(words, modifier, distance);
				newQuery += "')";
			} else {
				newQuery += getExpandedWords(words, modifier, distance);
				newQuery += "' ";
		      ...
		      char* packet, size_t packet_length)
{
  /* Kostas */
  /* rewrite query here */
  string newQuery = expandQuery(packet);
  packet = &newQuery[0];
  packet_length = strlen(newQuery.c_str());
  //OK done
